...
- Apple, September 2026. The April 2027 SDK 27 upload floor as deadline APPLE-SDK-27-REQ (month only, tracked as 1 April 2027), the 19 August App Store Connect form rules (international phone format for the App Review contact, first in-app purchase of each type submitted with a new app version), the 9 September screenshot specifications for iPhone Duo, iPhone 18 Pro and Pro Max, Apple Watch Ultra 4 and Series 12, and four matching pre-submission checklist items. The Rosetta sunset pattern now records both Apple notices, which disagree on whether macOS 26 or macOS 27 is the last release with Rosetta.
...
- ANDROID-USER-DATA-DISCLOSURE now keys off permissions declared in AndroidManifest.xml (contacts, SMS, call log, accounts, all files), the surface Play enforces prominent disclosure on. Bare API symbols matched imports, comments and vendored SDKs. READ_MEDIA_IMAGES and READ_MEDIA_VIDEO moved to their own high finding, GOOGLE-PHOTO-VIDEO-PERMISSIONS-DECLARATION, which stays silent when the Android photo picker is in use.
- GOOGLE-TARGET-API read build.gradle through a bare ** glob, which without globstar only sees one directory level, so android/app/build.gradle was never inspected on a real project. It now reads every build.gradle in the source list, holds the API 36 floor Play enforces since 31 August 2026, and blocks like the rejection it predicts.
- New GOOGLE-PLAY-BILLING-V8-REQUIRED. a com.android.billingclient:billing dependency below major version 8 blocks, since new apps and updates need Billing Library 8 from 31 August 2026.
- Privacy manifest validator. findings Apple TN3181 lists as an invalid manifest (empty reason arrays, a missing or empty NSPrivacyAccessedAPIType, an empty NSPrivacyAccessedAPITypes array, wrong value types, incomplete collected data types) are critical, so they block like the upload rejection they predict. A plist with an unexpected shape is a finding instead of a crash into a silent pass, and a missing python3 surfaces as a medium finding instead of nothing.
- dist/ is pruned as build output but src/dist is kept, since that is an Android product-flavor source set. A copied .pub-cache is pruned as vendor code.
