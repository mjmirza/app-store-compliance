...
- Apple Accessibility Nutrition Labels, introduced 2025, appear on the product page and cover 9 features (VoiceOver, Voice Control, Larger Text, Dark Interface, Differentiate Without Color Alone, Sufficient Contrast, Reduced Motion, Captions, Audio Descriptions). they are voluntary now, and Apple states they will become required to submit over time. a claimed feature must let users complete all common tasks with it, and a misleading label is contacted under Guideline 2.3 (accurate metadata), which is the enforceable accessibility hook today. Continuous iOS verification must cover:
  - **VoiceOver**: Setting descriptive non-empty labels (`accessibilityLabel`), hints, and appropriate semantic traits (`accessibilityTraits`) on all custom interactive elements and icons.
  - **Dynamic Type**: Avoiding hardcoded font sizes. Using preferred dynamic styles (e.g., `preferredFont(forTextStyle:)`) and ensuring `adjustsFontForContentSizeCategory = true` is declared on views.
  - **Reduce Motion**: Respecting system motion-reduction flag (`UIAccessibility.isReduceMotionEnabled`) in layout transitions and custom intensive animations, providing non-animating fallbacks.
  - **Color Contrast**: Ensuring high-contrast accessibility (WCAG 2.1 AA 4.5:1 ratio minimum) with semantic dynamic dynamic colors that automatically adjust between dark and light modes.
  - **Haptics**: Supplying audio-visual alternatives/notifications (`UIAccessibility.post`) for any critical haptic-only prompts or error vibrations.
  - **Keyboard navigation**: Supporting Switch Control and external hardware keyboard focus, utilizing focus indicators, focus arrays, and keyboard focus keys.
- Google Play provides tooling, not a gate. the Accessibility Scanner and TalkBack. checkable criteria include touch targets of at least 48dp, content descriptions on all interactive and graphical elements, and TalkBack-navigable controls. Continuous Android verification must cover:
  - **TalkBack**: Ensuring `android:contentDescription` is present and localized for all interactive layout nodes, leaving decorative assets explicitly marked empty/null.
  - **Font scaling**: Avoiding hardcoded pixel sizing (`dp`, `dip`, `px`) for text sizes. Always utilizing scale-independent pixel text size definitions (`sp`).
  - **High contrast**: Using robust contrast themes, testing against the High Contrast text feature in Android, and refraining from hardcoding low-contrast colors (like gray-on-gray colors).
  - **Accessibility Scanner recommendations**: Resolving standard Scanner complaints such as Touch Target Size constraints (verifying touch targets are at least 48dp x 48dp).
