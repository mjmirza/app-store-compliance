...
def main():
    ...
    if not overdue_warnings and not approaching_warnings:
        ...
        if overdue_warnings:
            ...
            md_lines.append("")
            md_lines.append("| Jurisdiction | Law | Requirement | Effective Date | Grace Period | Mandatory Date | Overdue Days | Enforcement Date | Affected Repository Sections | Priority |")
            md_lines.append("|---|---|---|---|---|---|---|---|---|---|")
            for item in overdue_warnings:
                overdue_days = abs(item["remaining_days"])
                md_lines.append(
                    f"| {item['jurisdiction']} | {item['law']} | {item['requirement']} | {item['effective_date']} | {item['grace_period']} | {item['mandatory_date']} | {overdue_days} days | {item['enforcement_date']} | {item['affected_repository_sections']} | {item['priority'].upper()} |"
                )
            md_lines.append("")
        ...
        if approaching_warnings:
            ...
            md_lines.append("")
            md_lines.append("| Jurisdiction | Law | Requirement | Effective Date | Grace Period | Mandatory Date | Days Remaining | Enforcement Date | Affected Repository Sections | Priority |")
            md_lines.append("|---|---|---|---|---|---|---|---|---|---|")
            for item in approaching_warnings:
                md_lines.append(
                    f"| {item['jurisdiction']} | {item['law']} | {item['requirement']} | {item['effective_date']} | {item['grace_period']} | {item['mandatory_date']} | {item['remaining_days']} days | {item['enforcement_date']} | {item['affected_repository_sections']} | {item['priority'].upper()} |"
                )
            md_lines.append("")
    ...
    md_lines.append("")
    md_lines.append("| Jurisdiction | Law | Requirement | Effective Date | Grace Period | Mandatory Date | Enforcement Date | Affected Repository Sections | Priority |")
    md_lines.append("|---|---|---|---|---|---|---|---|---|")
    for item in parsed_items:
        md_lines.append(
            f"| {item['jurisdiction']} | {item['law']} | {item['requirement']} | {item['effective_date']} | {item['grace_period']} | {item['mandatory_date']} | {item['enforcement_date']} | {item['affected_repository_sections']} | {item['priority'].upper()} |"
        )
    md_lines.append("")
