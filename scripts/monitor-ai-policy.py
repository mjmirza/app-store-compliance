...
def update_documentation(policy_matches, output_filepath, affected_features=None, is_simulated=True):
    """
    Appends the latest policy findings, affected features, and migration tasks directly to the output compliance file.
    """
    report_content = [
    ...
    report_content.append("## Identified Affected Features in Codebase")
    report_content.append("")
    if affected_features:
        unique_files = sorted(list(set(f["file"] for f in affected_features)))
        report_content.append(f"Scanned {len(affected_features)} AI integration signals across {len(unique_files)} files:")
        for uf in unique_files:
            report_content.append(f"- `{uf}`")
    else:
        report_content.append("No active AI integration signals detected in repository source files.")
...
def main():
    ...
    is_sim = not args.live
    update_documentation(matched_policies, args.output_docs, affected_features=affected_features, is_simulated=is_sim)
