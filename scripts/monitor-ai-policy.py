    ...
    for ann in announcements:
        ...
        if matched_kw:
            ...
            if platform == "Apple":
                ...
                if any(
                    x in text_to_search
                    for x in ["safety", "abuse", "nsfw", "moderation", "filter", "harmful", "deepfake", "report", "flag"]
                ):
                    review_items.append("Safety expectations")
                if any(
                    x in text_to_search
                    for x in ["disclosure", "consent", "modal", "inform", "disclose"]
                ):
...
def update_documentation(policy_matches, output_filepath, is_live=False):
    """
...
def main():
    ...
    update_documentation(matched_policies, args.output_docs, is_live=args.live)
