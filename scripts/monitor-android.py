#!/usr/bin/env python3
"""
Monitor all Android and Google Play requirements.
Tracks:
...
CATEGORY_KEYWORDS = {
    "Google Play Developer Policies": ["developer policy", "developer policies", "program policy", "google play policy", "policy center", "violat", "enforce"],
    "Play Console announcements": ["play console", "console announcement", "developer console", "identity verification", "verification", "announc"],
    "Target SDK requirements": ["target sdk", "targetsdkversion", "targetsdk", "api level requirement", "target api level", "android 15", "android 16", "android 17"],
    "Minimum SDK requirements": ["minimum sdk", "minsdkversion", "minsdk", "api level support", "legacy android", "min_sdk"],
    "Android API deprecations": ["deprecation", "deprecated api", "deprecated class", "deprecat", "legacy api", "safetynet retirement"],
    "Android permission model": ["android permission", "runtime permission", "permission model", "sensitive permission", "uses-permission", "requestpermissions"],
    "Background execution restrictions": ["background execution", "background work", "workmanager", "jobscheduler", "exact alarm", "alarmmanager", "wakelock"],
    "Foreground service policies": ["foreground service", "fgs type", "foregroundservice", "startforeground", "foregroundservicetype"],
    "Privacy Sandbox": ["privacy sandbox", "sandbox", "topics api", "protected audience", "sdk runtime", "attribution reporting"],
    "Play Integrity API": ["play integrity", "integrity api", "safetynet attestation", "attestation check", "integrity manager"],
    "Play Billing": ["play billing", "billing library", "billing client", "billing v8", "billing v9", "com.android.billingclient"],
    "User Data policy": ["user data", "data safety", "data deletion", "personal data", "privacy policy url", "account deletion"],
    "Data Safety section": ["data safety", "datasafety", "safety section", "data declaration", "data collection", "data sharing"],
    "AI-generated content policies": ["ai-generated", "generative ai", "ai content", "chatbot", "deepfake", "face-swap", "llm", "openai", "gemini"],
    "Accessibility requirements": ["accessibility", "talkback", "accessibility service", "wcag", "touch target", "content description", "bind_accessibility_service"],
    "Device compatibility requirements": ["device compatibility", "screen size", "tablet support", "compatible devices", "uses-feature", "foldable", "multi-window"],
    "Security Bulletins": ["security bulletin", "cve-", "vulnerability", "keystore", "ssl pinning", "key store", "biometrics", "cve"],
    "Android Enterprise requirements": ["android enterprise", "work profile", "managed device", "device policy manager", "device-admin", "deviceadminreceiver"],
    "Firebase policy updates": ["firebase", "google services", "crashlytics", "admob", "google-services.json"]
}

# Codebase signals (regex patterns) to find files affected by each of the 19 categories
CATEGORY_SIGNALS = {
    "Google Play Developer Policies": [r"Designed for Families", r"DesignedForFamilies", r"IARC", r"IARC_RATING"],
    ...
    "Android API deprecations": [r"SafetyNet", r"com.google.android.gms.ads.identifier", r"UIWebView"],
    "Android permission model": [r"requestPermissions", r"checkSelfPermission", r"Manifest\.permission", r"uses-permission"],
    "Background execution restrictions": [r"JobScheduler", r"WorkManager", r"AlarmManager", r"JobInfo", r"BroadcastReceiver"],
    "Foreground service policies": [r"FOREGROUND_SERVICE", r"startForeground", r"foregroundServiceType"],
    "Privacy Sandbox": [r"PrivacySandbox", r"AdSelectionManager", r"TopicsManager", r"ad-services", r"AdServicesOutlet"],
    "Play Integrity API": [r"PlayIntegrity", r"IntegrityManager", r"play-services-safetynet", r"IntegrityTokenRequest"],
    "Play Billing": [r"BillingClient", r"billingclient", r"billing-client", r"com\.android\.billingclient"],
    "User Data policy": [r"privacyPolicy", r"privacy-policy", r"privacy_policy", r"User Data", r"deleteAccount", r"delete_account"],
    "Data Safety section": [r"Data Safety", r"firebase-analytics", r"appsflyer", r"adjust", r"com\.facebook"],
    "AI-generated content policies": [r"api\.openai\.com", r"openai", r"gemini", r"generative", r"chat/completions", r"text-to-image", r"llm"],
    "Accessibility requirements": [r"AccessibilityService", r"BIND_ACCESSIBILITY_SERVICE", r"TalkBack", r"contentDescription"],
    "Device compatibility requirements": [r"uses-feature", r"uses-configuration", r"support-screens", r"supports-screens"],
    "Security Bulletins": [r"Security Bulletin", r"Android Keystore", r"KeyStore", r"biometrics", r"security-bulletin"],
    "Android Enterprise requirements": [r"device-admin", r"DeviceAdminReceiver", r"DevicePolicyManager", r"enterprise"],
    "Firebase policy updates": [r"firebase-", r"google-services\.json", r"crashlytics", r"FirebaseApp"]
}
...
MOCK_ANNOUNCEMENTS = [
    {
        ...
        "title": "Google Play Developer Program Policies Update 2026",
        "description": "Important updates to our Google Play Developer Program Policies regarding child-safety enforcement, mandatory content ratings (IARC), and strict guidelines preventing deceptive content promotions.",
...
def scan_codebase_for_android_signals(start_dir="."):
    ...
    for root, dirs, files in os.walk(start_dir):
        ...
        for file in files:
            ...
            if not file.endswith(('.kt', '.java', '.xml', '.gradle', '.kts', '.json', '.js', '.ts', '.md')):
                continue
...
def parse_rss_feed(url):
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={'User-Agent': 'Mozilla/5.0 (AndroidComplianceMonitor/1.0)'}
        )
...
def generate_pull_request_draft(updates, scan_results):
    ...
    for idx, u in enumerate(updates, 1):
        cat = u["category"]
        citations_list.append(f"- **{cat}**: [{u['title']}]({u['link']}) (Published: {u['pubDate']})")
        ...
        if cat == "Target SDK requirements":
            migration_steps.append(f"- **{cat}**: Update targetSdkVersion and compileSdkVersion in all build.gradle or build.gradle.kts files to API 36 (Android 16) before the August 31, 2026 deadline.")
            impl_checklist.append(f"- [ ] Update targetSdkVersion in build.gradle files to 36.")
            impl_checklist.append(f"- [ ] Verify all API 36 runtime behavioral changes do not impact application functionality.")
            risk_assessment.append(f"- *{cat}*: Publishing gate blockage for new submissions and updates if SDK target level is below requirement.")
        elif cat == "Minimum SDK requirements":
            migration_steps.append(f"- **{cat}**: Set minSdkVersion to 23 (Android 6.0) or higher to deprecate legacy API 21/22 support.")
            impl_checklist.append(f"- [ ] Update minSdkVersion to 23 in Gradle build configs.")
            risk_assessment.append(f"- *{cat}*: Deprecation of legacy devices leading to minor decrease in active user base.")
        elif cat == "Android API deprecations":
            migration_steps.append(f"- **{cat}**: Fully remove all legacy SafetyNet Attestation code references and complete migration to the Play Integrity SDK.")
            impl_checklist.append(f"- [ ] Remove 'com.google.android.gms:play-services-safetynet' dependency.")
            impl_checklist.append(f"- [ ] Implement Play Integrity token request flows on the client.")
            risk_assessment.append(f"- *{cat}*: Zero response/payload delivery for anti-abuse checks if legacy SafetyNet APIs are invoked.")
        elif cat == "Android permission model":
            migration_steps.append(f"- **{cat}**: Implement Scoped Media storage handling. Avoid broad READ_MEDIA_IMAGES/VIDEO requests; adopt the native Android Photo Picker instead.")
            impl_checklist.append(f"- [ ] Implement the native Android Photo Picker API wrapper.")
            impl_checklist.append(f"- [ ] Update AndroidManifest permissions; remove unnecessary broad media permissions.")
            risk_assessment.append(f"- *{cat}*: Runtime crash or automated play store rejection under the restricted user data policies.")
        elif cat == "Background execution restrictions":
            migration_steps.append(f"- **{cat}**: Ensure all WorkManager, JobScheduler, and AlarmManager tasks stay strictly within execution time limits. Validate SCHEDULE_EXACT_ALARM use cases.")
            impl_checklist.append(f"- [ ] Audit exact alarm declarations; replace with inexact alarms unless qualifies for exemption.")
            risk_assessment.append(f"- *{cat}*: Automated background service thottling or foreground service crash on target devices.")
        elif cat == "Foreground service policies":
            migration_steps.append(f"- **{cat}**: Declare precise foregroundServiceType properties on all <service> nodes in AndroidManifest.xml. Secure Play Console approval.")
            impl_checklist.append(f"- [ ] Declare correct foregroundServiceType in AndroidManifest.xml.")
            impl_checklist.append(f"- [ ] Draft play console foreground service declaration and record verification demo video.")
            risk_assessment.append(f"- *{cat}*: Missing console declarations will block release updates under Device and Network Abuse policy.")
        elif cat == "Privacy Sandbox":
            migration_steps.append(f"- **{cat}**: Migrate marketing/analytics workflows from legacy Advertising ID (GAID) tracking to the modern Privacy Sandbox Topics and Attribution APIs.")
            impl_checklist.append(f"- [ ] Update third-party tracking dependencies; configure privacy sandbox topics opt-in.")
            risk_assessment.append(f"- *{cat}*: Gradual tracking disruption as GAID is sunset across modern Android devices.")
        elif cat == "Play Integrity API":
            migration_steps.append(f"- **{cat}**: Implement secure server-side cryptographic nonce generation and verification for Play Integrity tokens.")
            impl_checklist.append(f"- [ ] Implement server-side Play Integrity token verification endpoint with cryptographic nonce checks.")
            risk_assessment.append(f"- *{cat}*: Vulnerability to replay attacks if integrity verdicts are not bound to a transaction-specific nonce.")
        elif cat == "Play Billing":
            migration_steps.append(f"- **{cat}**: Migrate billing modules to Google Play Billing Library version 8.0 or higher. Remove legacy BillingClient v7 or lower dependencies.")
            impl_checklist.append(f"- [ ] Upgrade billing client dependency 'com.android.billingclient:billing' to v8.0+.")
            risk_assessment.append(f"- *{cat}*: Blocked app updates post-August 31, 2026 if using outdated billing libraries.")
        elif cat == "User Data policy":
            migration_steps.append(f"- **{cat}**: Implement prominent user account and data deletion path in-app and publish a public, accessible web data deletion URL.")
            impl_checklist.append(f"- [ ] Build in-app 'Delete Account' UI option.")
            impl_checklist.append(f"- [ ] Publish public web data deletion form and enter URL in Play Console store listing.")
            risk_assessment.append(f"- *{cat}*: Rejection or store listing removal for failure to comply with the mandatory deletion url policy.")
        elif cat == "Data Safety section":
            migration_steps.append(f"- **{cat}**: Audit all integrated SDKs (Firebase, Facebook, AppsFlyer) and update the Google Play Console Data Safety questionnaire to exactly align with runtime actions.")
            impl_checklist.append(f"- [ ] Audit runtime network traffic from third-party SDKs.")
            impl_checklist.append(f"- [ ] Update Play Console Data Safety questionnaire declarations.")
            risk_assessment.append(f"- *{cat}*: Data Safety mismatch is the top Google Play rejection cause, threatening developer account health.")
        elif cat == "AI-generated content policies":
            migration_steps.append(f"- **{cat}**: Integrate content filters and prominent in-app disclosures for AI features. Provide one-click report/flag controls next to generated outputs.")
            impl_checklist.append(f"- [ ] Implement a prominent Play Policy disclosure dialog prior to accessing AI features.")
            impl_checklist.append(f"- [ ] Add flagging/reporting buttons directly adjacent to all generative AI content blocks.")
            risk_assessment.append(f"- *{cat}*: Immediate rejection or app suspension under Google Play's AI-generated content guidelines.")
        elif cat == "Accessibility requirements":
            migration_steps.append(f"- **{cat}**: Ensure all touch targets measure at least 48dp in physical size, and provide contentDescription tags on all interactive image elements.")
            impl_checklist.append(f"- [ ] Audit layout XML; verify all interactive targets measure >= 48dp.")
            impl_checklist.append(f"- [ ] Add contentDescription attributes on all ImageViews and ImageButtons.")
            risk_assessment.append(f"- *{cat}*: Increased store rejection risk and potential litigation under global digital accessibility laws.")
        elif cat == "Device compatibility requirements":
            migration_steps.append(f"- **{cat}**: Support dynamic window resizing, multi-window layout scaling, and foldable display orientations.")
            impl_checklist.append(f"- [ ] Configure android:resizeableActivity=true in manifest.")
            risk_assessment.append(f"- *{cat}*: Degraded UI experience and compatibility warnings on tablet/foldable devices.")
        elif cat == "Security Bulletins":
            migration_steps.append(f"- **{cat}**: Secure sensitive user secrets and credentials inside the hardware-backed Android Keystore system. Fix any outstanding vulnerability CVEs.")
            impl_checklist.append(f"- [ ] Implement cryptographic token storage wrapper backed by Android Keystore.")
            risk_assessment.append(f"- *{cat}*: Exposure of sensitive local storage secrets to side-channel extraction attacks.")
        elif cat == "Android Enterprise requirements":
            migration_steps.append(f"- **{cat}**: Secure managed device compliance. Implement DevicePolicyManager controls for work profiles.")
            impl_checklist.append(f"- [ ] Implement Work Profile boundaries; secure inter-profile communication.")
            risk_assessment.append(f"- *{cat}*: Non-compliance with corporate enterprise device management architectures.")
        elif cat == "Firebase policy updates":
            migration_steps.append(f"- **{cat}**: Migrate deprecated Firebase Dynamic Links configurations to Firebase Hosting deep links, App Links, or universal links.")
            impl_checklist.append(f"- [ ] Remove Firebase Dynamic Links dependency; migrate scheme to standard App Links.")
            risk_assessment.append(f"- *{cat}*: App onboarding or link share redirection failure post-sunset of dynamic links.")
        else:
            # Generic category
            migration_steps.append(f"- **{cat}**: Verify that all play console guidelines for {cat} are followed.")
            impl_checklist.append(f"- [ ] Double check Play Console compliance dashboard for {cat} notifications.")
...
def update_documentation_report(updates, output_filepath):
    ...
    for u in updates:
        ...
        lines.append(f"- **Regulatory Impact**: High priority. Publishing gates require action.")

        if cat == "Target SDK requirements":
            lines.append("- [ ] **Task 1**: Update `targetSdkVersion` in build.gradle files to 36.")
            lines.append("- [ ] **Task 2**: Test target API level 36 behaviors on devices.")
        elif cat == "Play Billing":
            lines.append("- [ ] **Task 1**: Migrate project dependencies to Billing Library version 8.0.")
            lines.append("- [ ] **Task 2**: Perform test transactions on Google Play console sandbox.")
        elif cat == "User Data policy":
            lines.append("- [ ] **Task 1**: Publish a public account and data deletion URL.")
            lines.append("- [ ] **Task 2**: Connect the URL to the Play Console User Data safety form.")
        elif cat == "Foreground service policies":
            lines.append("- [ ] **Task 1**: Specify foregroundServiceType inside the manifest service tags.")
            lines.append("- [ ] **Task 2**: Register foreground service type video verification demo on Play Console.")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Monitor all Android and Google Play Requirements")
    parser.add_argument("--live", action="store_true", help="Fetch live Google Play policy feeds")
    ...
    parser.add_argument("--output-docs", type=str, default="docs/ANDROID-POLICY-MIGRATION.md", help="Filepath to write migration tasks and logs")
    parser.add_argument("--pr-output", type=str, help="Filepath to save the drafted PR (outputs to stdout if omitted)")
    ...
    if args.live:
        ...
        announcements.extend(parse_rss_feed("https://source.android.com/security/bulletin.xml"))

    # Fallback to mock data if live has no updates or mock is explicitly requested
    if args.mock or (not args.live and not args.mock) or not announcements:
        print("Using comprehensive mock Android policy updates for compliance scanning...")
