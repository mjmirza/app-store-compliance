...
# Verifies Apple policy matching, codebase scanning,
...
# Execute scripts/monitor-apple.py with simulation
python3 scripts/monitor-apple.py --simulate "Privacy Manifests" --output-docs "$OUT_DOCS" --pr-output "$OUT_PR" > /tmp/monitor_apple_run.log 2>&1
RC=$?
...
if grep -q "Privacy Manifests" /tmp/monitor_apple_run.log; then
