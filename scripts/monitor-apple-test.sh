...
# Verifies mock RSS input parsing, 25 Apple requirement track keyword matching, codebase scanning,
# documentation generation, source trust hierarchy enforcement, and exact 15 non-vague sections in PR draft.
...
# Assert relevant tracks were matched
if grep -q "TRACK UPDATE: \[Privacy Manifests\]" /tmp/monitor_apple_run.log && grep -q "TRACK UPDATE: \[In-App Purchase policies\]" /tmp/monitor_apple_run.log; then
  ok "Correctly matched relevant Apple Developer requirement tracks"
else
  bad "Failed to match relevant Apple requirement tracks. Output: $(cat /tmp/monitor_apple_run.log)"
fi

# Assert documentation output
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "Privacy Manifests" "$OUT_DOCS" && grep -q "In-App Purchase policies" "$OUT_DOCS"; then
    ok "Documentation contains details of matched tracks"
  else
    bad "Documentation is missing track details"
  fi
else
