...
def write_migration_report(report_items, output_filepath):
    lines = [
    ...
    for item in report_items:
        ...
        for idx, step in enumerate(item["migration_tasks"], 1):
    ...
    try:
        with open(output_filepath, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
        print(f"[*] Documentation updated successfully at: {output_filepath}")
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--dir",
        help="Alias for --project (for compatibility with other scripts)",
    )
    parser.add_argument(
        "--simulate",
    ...
    # If matches were found, generate output files
    if report_items:
        # Generate the docs file
        os.makedirs(os.path.dirname(args.output_docs) or ".", exist_ok=True)
        write_migration_report(report_items, args.output_docs)

        # Generate the PR Draft file using the first matched item
        os.makedirs(os.path.dirname(args.pr_output) or ".", exist_ok=True)
        try:
            with open(args.pr_output, "w", encoding="utf-8") as f:
                # Compile all matched PRs or the first one, let's do the first or a consolidated one
                # Consolidating makes sense if multiple tracks match. Let's write the first one or a clean summary
                f.write(report_items[0]["proposed_pull_request"]["description"])
    ...
    if args.json:
        ...
        print_text_report(report_items, project_dir)
