...
writing documentation to docs/APPLE-POLICY-MIGRATION.md and a draft PR
to docs/APPLE_COMPLIANCE_PR_DRAFT.md without any emojis."""
...
def generate_pull_request(track_name, affected_files, item_title):
    ...
    if track_name in ["DMA compliance changes", "Alternative payment regulations"]:
        ...
        reg_change_desc = (
            ...
            "requirements and regional payment mandates."
        )
    else:
        reg_change_desc = (
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to write the migration report",
    )
    parser.add_argument(
        "--pr-output",
        default="docs/APPLE_COMPLIANCE_PR_DRAFT.md",
        help="Path to write the PR draft",
    )
    parser.add_argument(
    ...
    if args.json:
        ...
        # Generate docs/APPLE-POLICY-MIGRATION.md and docs/APPLE_COMPLIANCE_PR_DRAFT.md
        if report_items:
            ...
            try:
                ...
                print(f"Apple PR draft written successfully to: {args.pr_output}")
            except Exception as e:
                print(f"Failed to write Apple PR draft to {args.pr_output}: {e}", file=sys.stderr)
