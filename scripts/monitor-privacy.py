...
def classify_announcements(announcements, keywords_filter=None):
    ...
    Respects pre-defined categories to avoid cross-classification.
    """
    classified_updates = []
    ...
    for ann in announcements:
        ...
        # Prefer predefined category if present to avoid cross-classification
        predef = ann.get("category")
        if predef and predef in CATEGORIES:
            matched_categories = [predef]
        else:
...
def generate_pull_request_draft(updates, scan_results):
    ...
    Ensures unique, non-duplicate tasks, risk assessments, and migration steps.
    """
    citations_list = []
    ...
    seen_migration_categories = set()
    seen_checklist_categories = set()
...
def update_documentation_report(updates, output_filepath):
    ...
    Ensures unique task lists per category.
    """
    lines = [
    ...
    seen_tasks_categories = set()

    for u in updates:
        cat = u["category"]
        priority, is_verified = classify_source_and_verify(u)
        if priority in (4, 5) and not is_verified:
            if cat not in seen_tasks_categories:
        ...
        if cat not in seen_tasks_categories:
            ...
            if cat == "Privacy Manifest":
                ...
                lines.append("- [ ] **Task 1**: Ensure background location is backed by a persistent prominent in-app disclosure view.")
            elif cat == "Health permissions":
