...
def generate_pull_request_draft(updates, scan_results):
    ...
    seen_categories_steps = set()
    seen_categories_checklist = set()
    ...
    for idx, u in enumerate(updates, 1):
        ...
        if cat not in seen_categories_steps:
            seen_categories_steps.add(cat)
        ...
        if cat not in seen_categories_checklist:
            seen_categories_checklist.add(cat)
            if cat == "Privacy Manifest":
...
def update_documentation_report(updates, output_filepath):
    ...
    seen_categories_tasks = set()
    for u in updates:
        cat = u["category"]
        priority, is_verified = classify_source_and_verify(u)
        if priority in (4, 5) and not is_verified:
            if cat not in seen_categories_tasks:
                seen_categories_tasks.add(cat)
