...
if grep -q "NIST Secure Storage Revisions" /tmp/monitor_security_run.log || grep -q "Biometric Bypass Vulnerabilities" /tmp/monitor_security_run.log || grep -q "NIST" docs/SECURITY-POLICY-MIGRATION.md; then
