...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO/IEC 27001:2022 Security Controls Guidance",
    "description": "Information Security Management System controls for cloud and mobile applications.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Application Security Verification Standard Update",
    "description": "OWASP releases updated verification controls for application security.",
...
if grep -q "ISO/IEC 27001" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO/IEC 27001" "$OUT_DOCS" && grep -q "OWASP MASVS" "$OUT_DOCS"; then
