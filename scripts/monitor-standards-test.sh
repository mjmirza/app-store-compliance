...
[
  {
    ...
    "description": "Crucial updates requiring all information security management systems to audit Annex A controls.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Mobile Security Updates",
    "description": "OWASP MASVS controls require strict application anti-tampering and network pinning.",
...
if grep -q "ISO 27001 Annex A" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Mobile Security" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Annex A" "$OUT_DOCS" && grep -q "OWASP MASVS Mobile Security" "$OUT_DOCS"; then
