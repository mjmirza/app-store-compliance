...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 ISMS Requirements Update",
    "description": "Essential ISO 27001 regulations require robust access controls and security logging.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Guidelines Update",
    "description": "All mobile and web applications must adhere to OWASP MASVS verification standards.",
...
if grep -q "ISO 27001 ISMS Requirements" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Guidelines" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 ISMS Requirements" "$OUT_DOCS" && grep -q "OWASP MASVS Guidelines" "$OUT_DOCS"; then
