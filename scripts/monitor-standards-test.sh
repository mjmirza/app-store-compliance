...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Information Security Management Standard Guideline Update",
    "description": "Essential ISMS updates require refined Annex A controls and information security access policies.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP Application Security Guidelines Update",
    "description": "All submitted applications must adhere to OWASP MASVS and ASVS controls.",
...
echo "== Running Standards Policy Monitor Test Suite =="
...
if grep -q "ISO 27001 Information Security Management" /tmp/monitor_standards_run.log && grep -q "OWASP Application Security Guidelines" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Information Security Management" "$OUT_DOCS" && grep -q "OWASP Application Security Guidelines" "$OUT_DOCS"; then
...
echo ""
echo "Standards Policy Monitor test suite: $PASS passed, $FAIL failed"
if [ "$FAIL" -eq 0 ]; then
  exit 0
else
  exit 1
