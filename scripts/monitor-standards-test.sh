...
# Verifies mock input parsing, technical standards matching, codebase scanning,
# documentation generation, source trust hierarchy enforcement, and the presence
# of exactly 15 required non-vague sections in the PR draft.
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO/IEC 27001 ISMS Requirements Update",
    "description": "Crucial updates requiring ISMS access control and cloud encryption alignment.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Security Standard Revision",
    "description": "OWASP MASVS controls require strict token storage and input sanitization.",
  ...
  },
  {
    ...
    "title": "NIST AI RMF 1.0 Governance Framework",
    "description": "NIST releases updated AI Risk Management Framework guidance.",
...
if grep -q "ISO/IEC 27001 ISMS Requirements" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Security Standard" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO/IEC 27001 ISMS Requirements" "$OUT_DOCS" && grep -q "OWASP MASVS Security Standard" "$OUT_DOCS"; then
    ok "Documentation contains details of matched standards updates"
  else
    bad "Documentation is missing standards details"
  fi
else
