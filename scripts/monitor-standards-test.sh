...
# Verifies mock input parsing, standards keyword classification, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Security Controls Transition",
    "description": "Crucial updates requiring ISMS alignment with updated Annex A controls.",
  ...
  },
  {
    ...
    "title": "NIST AI Risk Management Framework Guidelines",
    "description": "NIST AI RMF guidance across Govern Map Measure Manage pillars.",
...
if grep -q "ISO 27001 Security Controls" /tmp/monitor_standards_run.log && grep -q "NIST AI Risk Management" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Security Controls" "$OUT_DOCS" && grep -q "NIST AI Risk Management" "$OUT_DOCS"; then
