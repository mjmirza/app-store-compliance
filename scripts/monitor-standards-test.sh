...
# Verifies mock RSS/JSON input parsing, technical standards keyword matching, codebase scanning,
...
# Create a mock dataset covering technical standards
cat << 'EOF' > "$MOCK_JSON"
[
  {
    "id": "MOCK-STD-ISO-27001",
    "category": "ISO 27001",
    "title": "ISO/IEC 27001 ISMS Access Control Mandate",
    "description": "Crucial updates requiring ISMS access control policy alignment and threat monitoring.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Mobile Application Verification",
    "description": "OWASP updates MASVS storage and network communication requirements.",
  ...
  },
  {
    ...
    "title": "NIST AI Risk Management Framework Standards",
    "description": "NIST AI RMF core updates for trustworthy AI governance and measure functions.",
...
if grep -q "ISO/IEC 27001 ISMS" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS" /tmp/monitor_standards_run.log; then
...
# Assert documentation was generated
if [ -f "$OUT_DOCS" ]; then
  ok "Documentation output file created at $OUT_DOCS"
  if grep -q "ISO/IEC 27001 ISMS" "$OUT_DOCS" && grep -q "OWASP MASVS" "$OUT_DOCS"; then
