...
# gap identification, documentation generation, and the presence of exactly 15 required non-vague sections in the PR draft.
# Also verifies that all generated outputs and scripts are 100% emoji-free.
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 ISMS Controls Revision Guidance",
    "description": "Essential updates requiring alignment of access control policy and information security governance.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Mobile Security Standards Release",
    "description": "OWASP MASVS v2.1 updates security requirements across storage, crypto, auth, and network controls.",
...
if grep -q "ISO 27001 ISMS Controls" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Mobile Security" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 ISMS Controls" "$OUT_DOCS" && grep -q "OWASP MASVS Mobile Security" "$OUT_DOCS"; then
