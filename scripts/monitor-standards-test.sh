...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Security Controls Update",
    "description": "Crucial updates requiring all ISMS controls to align with revised Annex A policies.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS 2.1 Verification Standard",
    "description": "OWASP MASVS mobile security controls update requiring anti-tampering and secure storage.",
...
if grep -q "ISO 27001 Security Controls" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS 2.1" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Security Controls" "$OUT_DOCS" && grep -q "OWASP MASVS 2.1" "$OUT_DOCS"; then
