...
[
  {
    "id": "STD-TEST-ISO27001",
    "category": "ISO 27001",
    "title": "ISO 27001 Security Standard Guidance Update",
    "description": "Essential ISMS regulations require robust Annex A controls.",
  ...
  },
  {
    "id": "STD-TEST-NISTCSF",
    "category": "NIST CSF",
    "title": "NIST Cybersecurity Framework 2.0 Integration",
    "description": "NIST CSF 2.0 mandates governance and supply chain risk management.",
...
if grep -q "ISO 27001 Security Standard" /tmp/monitor_standards_run.log && grep -q "NIST Cybersecurity Framework" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Security Standard" "$OUT_DOCS" && grep -q "NIST Cybersecurity Framework" "$OUT_DOCS"; then
