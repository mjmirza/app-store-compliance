...
[
  {
    ...
    "description": "Crucial updates requiring ISMS policies to align with ISO 27001 Annex A controls.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS v2.1 Verification Standard Release",
    "description": "OWASP updates mobile security controls for client-side storage and network pinning.",
...
# Assert relevant standards policies were matched
if grep -q "ISO/IEC 27001:2022 Controls Alignment Update" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS v2.1 Verification Standard Release" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001" "$OUT_DOCS" && grep -q "OWASP" "$OUT_DOCS"; then
