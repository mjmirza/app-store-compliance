...
# Verifies mock input parsing, technical standards keyword matching, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO/IEC 27001 Threat Intelligence Guidance Update",
    "description": "Essential ISO 27001 rules require robust ISMS threat tracking.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Storage and Cryptography Updates",
    "description": "Mobile security guidelines mandate hardware-backed keystores.",
...
if grep -q "ISO/IEC 27001 Threat Intelligence" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Storage" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO/IEC 27001 Threat Intelligence" "$OUT_DOCS" && grep -q "OWASP MASVS Storage" "$OUT_DOCS"; then
