...
# Verifies mock input parsing, 10 Technical Standards keyword matching, codebase scanning,
# documentation report generation (gaps, tasks, docs, tests), and the presence of exactly 15 required non-vague sections in the PR draft.
# Also verifies that all generated outputs and scripts are completely emoji-free.
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Annex A Security Controls Update",
    "description": "Essential updates requiring ISMS alignment and threat intelligence protocols.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Hardware Key Enclave Controls",
    "description": "All mobile applications must store keys in hardware enclaves and implement certificate pinning.",
...
if grep -q "ISO 27001 Annex A" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Hardware" /tmp/monitor_standards_run.log; then
...
# Assert that documentation report was generated and contains key sections
if [ -f "$OUT_DOCS" ]; then
  ok "Documentation output file created at $OUT_DOCS"
  if grep -q "ISO 27001 Annex A" "$OUT_DOCS" && \
     grep -q "Identified Repository Gaps" "$OUT_DOCS" && \
     grep -q "Implementation Tasks" "$OUT_DOCS" && \
     grep -q "Documentation Updates" "$OUT_DOCS" && \
     grep -q "Testing Updates" "$OUT_DOCS"; then
    ok "Documentation contains repository gaps, implementation tasks, documentation updates, and testing updates"
  else
    bad "Documentation is missing required gap/task/doc/test detail sections"
  fi
else
