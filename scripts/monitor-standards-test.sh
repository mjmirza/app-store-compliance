...
# documentation generation (including testing updates), source trust hierarchy blocking,
# and the presence of exactly 15 required non-vague sections in the PR draft.
...
[
  {
    "id": "STANDARDS-MOCK-TEST-ISO27001",
    "category": "ISO 27001",
    "title": "ISO 27001 Security Management Update",
    "description": "Essential updates requiring alignment of Annex A security controls and ISMS policies.",
    "link": "https://www.iso.org/standard/27001",
    "pubDate": "Fri, 15 May 2026 10:00:00 PDT"
  },
  {
    "id": "STANDARDS-MOCK-TEST-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Mobile Application Security Verification Standard",
    "description": "Updated baseline requirements for mobile application security verification.",
    "link": "https://owasp.org/www-project-mobile-app-security/",
    "pubDate": "Sat, 16 May 2026 11:00:00 PDT"
  },
  {
    "id": "STANDARDS-MOCK-TEST-UNVERIFIED",
    "category": "ISO 27001",
    "title": "Unverified Industry Rumors on ISO 27001",
    "description": "Unverified blog rumor about ISO certificates.",
    "link": "https://randomblogsite.com/iso-rumor",
    "pubDate": "Sun, 17 May 2026 12:00:00 PDT"
  }
]
EOF
...
if grep -q "ISO 27001 Security Management" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Mobile Application" /tmp/monitor_standards_run.log; then
...
# Assert source trust hierarchy blocked unverified Priority 4 source
if grep -q "ALERT \[Source Trust Hierarchy\]: Unverified Priority 4 source detected" /tmp/monitor_standards_run.log; then
  ok "Source trust hierarchy correctly alerted and blocked unverified secondary source"
else
  bad "Source trust hierarchy failed to alert on unverified secondary source. Log output: $(cat /tmp/monitor_standards_run.log)"
fi

# Assert that documentation was generated and contains testing updates
if [ -f "$OUT_DOCS" ]; then
  ok "Documentation output file created at $OUT_DOCS"
  if grep -q "ISO 27001 Security Management" "$OUT_DOCS" && grep -q "OWASP MASVS Mobile Application" "$OUT_DOCS"; then
    ok "Documentation contains details of matched policy updates"
  else
    bad "Documentation is missing policy details"
  fi

  if grep -q "Testing Updates Summary" "$OUT_DOCS" && grep -q "Task 2 (Testing Update)" "$OUT_DOCS"; then
    ok "Documentation includes testing updates"
  else
    bad "Documentation is missing testing updates section"
  fi
else
...
if check_no_emojis "scripts/monitor-standards.py" && \
   check_no_emojis "scripts/monitor-standards-test.sh" && \
   check_no_emojis "$OUT_DOCS" && \
   check_no_emojis "$OUT_PR"; then
