...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO/IEC 27001 ISMS Standard Update",
    "description": "Crucial updates to information security management systems.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Benchmark Guidance",
    "description": "Mobile application security verification standard controls update.",
...
if grep -q "ISO/IEC 27001 ISMS Standard" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Benchmark" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO/IEC 27001 ISMS Standard" "$OUT_DOCS" && grep -q "OWASP MASVS Benchmark" "$OUT_DOCS"; then
