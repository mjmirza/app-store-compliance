...
# Verifies mock JSON input parsing, standards keyword matching, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO/IEC 27001 Information Security Management Revision",
    "description": "Crucial updates to ISMS guidelines mandating strict information security control checks.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP Security Policies",
    "description": "Updated MASVS client-side local storage and vulnerability guidelines.",
...
if grep -q "ISO/IEC 27001 Information Security" /tmp/monitor_standards_run.log && grep -q "OWASP Security Policies" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO/IEC 27001 Information Security" "$OUT_DOCS" && grep -q "OWASP Security Policies" "$OUT_DOCS"; then
