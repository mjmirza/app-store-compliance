...
# Verifies mock JSON input parsing, Technical Standards keyword matching, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Security Controls Specification",
    "description": "Essential ISO 27001 regulations require robust ISMS governance and access control mechanisms.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Guidelines Refresh",
    "description": "OWASP mobile application security controls mandate strict MASVS compliance.",
...
if grep -q "ISO 27001 Security Controls" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Guidelines" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Security Controls" "$OUT_DOCS" && grep -q "OWASP MASVS Guidelines" "$OUT_DOCS"; then
