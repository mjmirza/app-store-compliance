...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Information Security Controls Update",
    "description": "Essential ISO 27001 regulations require robust ISMS governance and control auditing.",
  ...
  },
  {
    ...
    "category": "OWASP",
    "title": "OWASP MASVS Security Verification Standard Release",
    "description": "All submitted apps must adhere to OWASP MASVS client security controls.",
...
if grep -q "ISO 27001 Information Security" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Security" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Information Security" "$OUT_DOCS" && grep -q "OWASP MASVS Security" "$OUT_DOCS"; then
...
# Execute custom tool verification
if [ -x "/home/jules/self_created_tools/verify_standards_output.py" ]; then
  python3 /home/jules/self_created_tools/verify_standards_output.py "$OUT_PR" "docs/STANDARDS-POLICY-MIGRATION.md" > /dev/null 2>&1
  if [ $? -eq 0 ]; then
    ok "Custom tool verify_standards_output.py verified PR sections and 10 standards coverage"
  else
    bad "Custom tool verify_standards_output.py failed"
  fi
fi
