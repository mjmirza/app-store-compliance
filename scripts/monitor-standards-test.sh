...
# Verifies mock input parsing, standards classification, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Standard Control Update",
    "description": "Crucial updates requiring all ISMS controls to align with Annex A threat management.",
  ...
  },
  {
    ...
    "title": "NIST AI RMF Core Function Guidance",
    "description": "NIST guidance for AI risk management framework core functions.",
...
if grep -q "ISO 27001 Standard Control Update" /tmp/monitor_standards_run.log && grep -q "NIST AI RMF Core Function Guidance" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Standard Control Update" "$OUT_DOCS" && grep -q "NIST AI RMF Core Function Guidance" "$OUT_DOCS"; then
