...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 ISMS Security Control Framework update",
    "description": "Information Security Management System updates regarding climate actions and risk assessments.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS 2026 Mobile Application Security Verification",
    "description": "OWASP MASVS updates enforcing hardware-backed token storage and strict certificate pinning.",
...
if grep -q "ISO 27001 ISMS Security" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS 2026" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 ISMS Security" "$OUT_DOCS" && grep -q "OWASP MASVS 2026" "$OUT_DOCS"; then
...
if [ -f "$OUT_PR" ]; then
  ...
  # Verify strict emoji-free policy on output and script
  EMOJI_CHECK=$(python3 -c "
import sys
for filepath in ['$OUT_PR', 'scripts/monitor-standards.py', 'scripts/monitor-standards-test.sh']:
    try:
