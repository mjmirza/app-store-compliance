...
# Test 4: Run simulation for ISO 27001 and verify output contains the 15 required sections in JSON
echo "[TEST] Simulating 'ISO 27001' standard and validating 15-section JSON output..."
ISO_JSON=$(python3 "$MON_SCRIPT" --project "$REPO_ROOT" --simulate "ISO 27001" --json)
...
for sect in "${SECTIONS[@]}"; do
  ...
  if ! echo "$ISO_JSON" | grep -q "## $sect"; then
...
JSON_OUT=$(python3 "$MON_SCRIPT" --project "$REPO_ROOT" --simulate "ISO 27001" --json)
...
RUMOR_JSON=$(python3 "$MON_SCRIPT" --project "$REPO_ROOT" --simulate "social media" --json)

# Verify that RUMOR_JSON has "proposed_pull_request": null
if ! echo "$RUMOR_JSON" | grep -q '"proposed_pull_request": null'; then
  echo "[ERROR] Expected social media rumor (Priority 5) to be blocked (proposed_pull_request: null)"
  exit 1
fi
...
if echo "$ISO_JSON" | grep -q '"proposed_pull_request": null'; then
  echo "[ERROR] Expected ISO 27001 (Priority 1) to generate a Pull Request, but it was blocked"
  exit 1
fi
