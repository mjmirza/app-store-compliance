...
# Verifies mock input parsing, standards category matching, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 ISMS Amendment",
    "description": "Crucial updates to ISO/IEC 27001 requiring Statement of Applicability updates.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Revision",
    "description": "OWASP MASVS controls require SPKI pinning and secure key storage.",
...
if grep -q "ISO 27001 ISMS Amendment" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Revision" /tmp/monitor_standards_run.log; then
  ok "Correctly matched relevant standards updates"
else
  bad "Failed to match relevant standards updates. Output: $(cat /tmp/monitor_standards_run.log)"
fi

if [ -f "$OUT_DOCS" ]; then
  ok "Documentation output file created at $OUT_DOCS"
  if grep -q "ISO 27001 ISMS Amendment" "$OUT_DOCS" && grep -q "OWASP MASVS Revision" "$OUT_DOCS"; then
