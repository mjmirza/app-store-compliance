...
# Verifies mock RSS input parsing, keyword matching, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Information Security Management Standard Control Update",
    "description": "Crucial updates requiring all local user databases to follow ISMS controls and access management.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Guidelines Release",
    "description": "OWASP verification controls require hardware token storage and anti-tampering verification.",
...
if grep -q "ISO 27001 Information Security" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Guidelines" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Information Security" "$OUT_DOCS" && grep -q "OWASP MASVS Guidelines" "$OUT_DOCS"; then
