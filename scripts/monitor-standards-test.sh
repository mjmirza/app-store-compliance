...
# Verifies mock standards input parsing, keyword matching, codebase scanning,
...
[
  {
    ...
    "category": "ISO 27001",
    "title": "ISO 27001 Annex A Updates on Access Control",
    "description": "Crucial revisions requiring clear isolation of access control mechanisms and local databases.",
  ...
  },
  {
    "id": "MOCK-STD-OWASP",
    "category": "OWASP",
    "title": "OWASP MASVS Secure Local Storage Baseline",
    "description": "Updated baseline parameters for local storage encryption.",
...
if grep -q "ISO 27001 Annex A" /tmp/monitor_standards_run.log && grep -q "OWASP MASVS Secure" /tmp/monitor_standards_run.log; then
...
if [ -f "$OUT_DOCS" ]; then
  ...
  if grep -q "ISO 27001 Annex A" "$OUT_DOCS" && grep -q "OWASP MASVS Secure" "$OUT_DOCS"; then
