...
def generate_pull_request(track_name, affected_files, item_title):
    ...
    meta = TRACK_METADATA.get(track_name, {
    ...
    elif track_name in ["Apple Privacy requirements", "Privacy Manifests", "Required Reason APIs", "App Tracking Transparency"]:
        reg_change_desc = (
    ...
    migration_steps_lines.append(f"1. Conduct a codebase audit focusing on keywords/APIs matching: `{meta['detect_regex']}`")
    for step in meta["migration_steps"]:
        migration_steps_lines.append(f"2. {step}")
    migration_steps_lines.append("3. Run the automated pre-submission compliance guard (`bash agent-os/hooks/app-store-compliance-guard.sh .`) to verify that the changes satisfy all local verification criteria.")
    ...
    compliance_impact_desc = (
        f"Implementing this change protects our developer standing, aligning the application with global regulatory frameworks "
        f"and platform requirements. Successful implementation reduces our App Store submission risk profile to **Low** and "
        f"ensures we remain in good legal standing across our entire operational user base."
    )
