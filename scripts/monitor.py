#!/usr/bin/env python3
"""
Monitor platform-specific AI policies for Apple and Google Play.
Scans live RSS/Atom feeds or local mock data, identifies affected codebase features,
updates/generates compliance documentation, and drafts a comprehensive 15-section
Pull Request.
"""
...
COMPLIANCE_DOMAINS = [
    "AI-Generated Content", "App Review AI Guidance", "Safety Expectations", "User Disclosure",
    "Privacy Manifest", "Required Reason APIs", "Tracking Domains", "Export Compliance",
    "Trader Status", "In-App Purchases", "Restore Purchases", "Subscriptions",
    "Demo Accounts", "Human Interface Guidelines", "GDPR Compliance", "COPPA Compliance",
    "EU AI Act", "Designed for Families", "Simulated Gambling", "Loot Boxes",
    "macOS Sandboxing", "Dynamic Code Loading", "Package Visibility", "Overlays/Tapjacking",
    "Account Deletion"
]

# AI signals to look for when scanning the codebase
AI_SIGNALS = [
    r"api\.openai\.com", r"openai", r"anthropic", r"gemini", r"generativelanguage",
    r"chat/completions", r"stable[ -_]diffusion", r"text-to-image", r"image generation",
    r"chatgpt", r"claude", r"llm", r"generative[ -_]ai"
]
...
MOCK_ANNOUNCEMENTS = [
    {
        ...
        "platform": "Apple",
        "title": "Update on App Review Guidelines: Generative AI Safety and User Disclosures",
        "description": "Today we are updating Guideline 1.2 and 5.1.2. Apps with generative AI features must implement robust, real-time input/output moderation and user-reporting mechanisms. Any personal data shared with third-party LLM providers must be preceded by an explicit, prominent user disclosure and consent modal. AI chatbot outputs must count towards the content age rating questionnaire.",
        "link": "https://developer.apple.com/news/?id=ai_safety_2026",
        "pubDate": "Wed, 01 Apr 2026 10:00:00 PDT"
    },
    {
        "id": "GOOGLE-AI-2026-POLICY",
        "platform": "Google Play",
        "title": "New Google Play Policy: AI-Generated Content Disclosures and Safeguards",
        "description": "Beginning soon, Google Play will require developers of apps featuring generative AI to provide prominent in-app AI-generated content disclosures, allow users to flag/report offensive generated content, and block bad actors. Developers must prevent the generation of deepfakes, face-swaps, and non-consensual sexual content, ensuring robust user safety safeguards are active.",
        "link": "https://android-developers.googleblog.com/2026/04/google-play-ai-policy.html",
...
def scan_codebase(start_dir="."):
    ...
    for root, dirs, files in os.walk(start_dir):
        ...
        for file in files:
            ...
            if not file.endswith(('.swift', '.m', '.h', '.kt', '.java', '.xml', '.plist', '.gradle', '.kts', '.json', '.js', '.ts', '.dart', '.md')):
                continue
...
def parse_rss_feed(url):
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={'User-Agent': 'Mozilla/5.0 (AppStoreComplianceMonitor/1.0)'}
        )
...
def analyze_announcements(announcements, keywords):
    ...
    for ann in announcements:
        ...
        if matched_kw:
            ...
            platform = "Apple" if "apple" in text_to_search or ann.get("platform") == "Apple" else "Google Play"

            # Map specific review requirements
            review_items = []
            if platform == "Apple":
                if any(x in text_to_search for x in ["content", "generative", "output", "model"]):
                    review_items.append("AI-generated content requirements")
                if any(x in text_to_search for x in ["review", "guideline", "guidance"]):
                    review_items.append("App Review AI guidance")
                if any(x in text_to_search for x in ["safety", "abuse", "nsfw", "moderation", "filter"]):
                    review_items.append("Safety expectations")
                if any(x in text_to_search for x in ["disclosure", "consent", "modal", "inform"]):
                    review_items.append("User disclosure requirements")
                if not review_items:
                    review_items = ["General Apple AI policy"]
            else:
                if any(x in text_to_search for x in ["policy", "guideline", "requirement"]):
...
def generate_pull_request_draft(policy_matches, affected_files):
    ...
    citations_list = "\n".join(f"- [{m['title']}]({m['link']}) ({m['platform']} Update, {m['pubDate']})" for m in policy_matches)
    ...
    if "Apple" in platforms_triggered:
        migration_steps_list.append("1. **Consent Modal**: Add an in-app consent modal detailing that third-party AI/LLM components are used and get explicit consent before sending user personal data.")
        migration_steps_list.append("2. **Output Moderation**: Wire real-time prompt/response filters to detect, flag, and filter out objectionable or NSFW AI content.")
        migration_steps_list.append("3. **Age Rating Update**: Update the age rating questionnaire in App Store Connect to account for interactive AI chat functionality.")

        impl_checklist_items.append("- [ ] Create `ConsentModalView` and integrate it into onboarding/settings.")
        impl_checklist_items.append("- [ ] Integrate OpenAI/Anthropic moderation API or client-side bad-word list.")
        impl_checklist_items.append("- [ ] Add reporting and content flag buttons next to AI-generated messages.")
        impl_checklist_items.append("- [ ] Recheck App Store Connect questionnaire for Guideline 1.2 and 2.3.6 updates.")
    if "Google Play" in platforms_triggered or not platforms_triggered:
        migration_steps_list.append("4. **Prominent Disclosure**: Implement an in-app disclaimer and user consent sheet for generative content on Android devices.")
        migration_steps_list.append("5. **Content Safety Controls**: Add a prominent 'report content' or 'flag output' UI element directly on all AI output cards.")
        migration_steps_list.append("6. **Terms of Service update**: Declare user safety requirements regarding deepfakes and non-consensual content generation.")

        impl_checklist_items.append("- [ ] Implement a prominent Play Policy disclosure dialog on app launch or AI feature access.")
        impl_checklist_items.append("- [ ] Implement one-click reporting next to every AI output block on Android.")
        impl_checklist_items.append("- [ ] Prevent face-swap and image generation capabilities if NSFW/deepfake models can be accessed.")
        impl_checklist_items.append("- [ ] Update the Google Play Console Data Safety form declarations.")
...
def main():
    parser = argparse.ArgumentParser(description="App Store & Google Play AI Policy Monitor")
    parser.add_argument("--live", action="store_true", help="Fetch live RSS/Atom feeds from Apple and Google blog")
    parser.add_argument("--mock", type=str, help="Path to a mock JSON/XML file or 'inline' to use built-in mock policy updates")
    parser.add_argument("--keywords", type=str, default="ai,generative,artificial intelligence,llm,openai,anthropic,gemini,safety,user disclosure,disclosure",
    ...
    parser.add_argument("--output-docs", type=str, default="docs/AI-POLICY-MIGRATION.md", help="Filepath to write migration tasks / docs updates")
    parser.add_argument("--pr-output", type=str, help="Filepath to save the drafted PR (will output to stdout if omitted)")
    ...
    if args.mock or (not args.live and not args.mock):
        ...
        if args.mock and args.mock != "inline" and os.path.exists(args.mock):
            try:
                ...
                print(f"Failed to read mock file {args.mock}: {e}, falling back to inline data", file=sys.stderr)
