#!/usr/bin/env python3
"""
Release Readiness Compliance Audit Engine.
Performs a complete compliance audit before every release, validating internal engines,
running metadata and guard scans, and compiling a comprehensive markdown report.
Blocks release (exits non-zero) if any critical compliance issue is identified.
"""
...
MAP_PATTERNS_TO_AREAS = {
    ...
    "APPLE-5.1.2-AI-NO-CONSENT-MODAL": ["Apple requirements", "AI regulations", "Privacy"],
...
def find_affected_files(target_dir, patterns_dict):
    ...
    allowed_exts = {
        ".swift", ".m", ".h", ".kt", ".java", ".xml", ".plist", ".gradle",
        ".kts", ".json", ".js", ".ts", ".dart", ".xcconfig", ".pbxproj", ".entitlements", ".md"
    }

    for root, dirs, files in os.walk(target_dir):
        # modify dirs in place to prune excluded dirs
        dirs[:] = [d for d in dirs if d not in exclude_dirs and not d.endswith("Tests") and not d.endswith("Tests__")]
        ...
        for file in files:
            ...
            for pid, pdata in patterns_dict.items():
                ...
                # For BOTH-PLACEHOLDER, we need special regexes because signals in rejection-patterns.json
                # are simple word checks, but in the shell guard it uses a more refined list.
                # Let's perform precise match.
                has_signal = False
                ...
                if has_signal:
                    ...
                    if not has_counter:
                        ...
                        if "rejection-patterns.json" not in rel_path and "release-audit.py" not in rel_path and "app-store-compliance-guard.sh" not in rel_path:
...
def main():
    ...
    guard_test_code, guard_test_out, guard_test_err = run_command("bash agent-os/hooks/app-store-compliance-guard-test.sh")
    ...
    meta_test_code, meta_test_out, meta_test_err = run_command("bash scripts/metadata-audit-test.sh")
    if meta_test_code != 0:
    ...
    guard_code, guard_out, guard_err = run_command(f"bash agent-os/hooks/app-store-compliance-guard.sh {target_dir}")

    # Run metadata-audit.py
    # If en-US metadata exists, use it, otherwise let it run with default empty metadata scan
    meta_code, meta_out, meta_err = run_command(f"python3 scripts/metadata-audit.py {target_dir}")
    ...
    while i < len(lines):
        line = lines[i]
        match = re.match(r'^\s*\[(CRITICAL|HIGH|MEDIUM|LOW)\]\s+([A-Z0-9-._]+)\s+(.+)$', line, re.IGNORECASE)
        if match:
    ...
    if has_critical:
        report_lines.append("The release is currently BLOCKED due to one or more critical compliance issues that must be resolved before submitting to the platforms.")
    elif findings:
        report_lines.append("The release is ready but has outstanding non-critical advisory risks. Review the required actions and consult the recommended reviewers before finalizing the release.")
    else:
        report_lines.append("The release has successfully passed all compliance audits with zero outstanding risks. Ready for deployment.")
    report_lines.append("")
    ...
    for idx, area in enumerate(REQUIRED_AREAS, 1):
        ...
        report_lines.append(f"- Recommended Reviewers: {RECOMMENDED_REVIEWERS.get(area, 'Lead Developer')}")
        report_lines.append("")
    ...
    # Write report file
    report_path = os.path.join(ROOT, "RELEASE-READINESS-REPORT.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines) + "\n")

    print(f"Release readiness report generated successfully at: {report_path}")
    print(f"Summary: critical={sum(1 for f in findings if f['severity'] == 'critical')} high={sum(1 for f in findings if f['severity'] == 'high')} medium={sum(1 for f in findings if f['severity'] == 'medium')} low={sum(1 for f in findings if f['severity'] == 'low')}")
