#!/usr/bin/env python3
"""
Test suite for deadline-checker.py.
Simulates loading a mock database of deadlines with:
  - An overdue/passed deadline
  - An upcoming deadline (e.g. 5 days remaining)
  - A far future deadline (e.g. 200 days remaining)
Verifies the output matches the expected behavior and has no emojis.
"""
import unittest
import os
import sys
...
class TestDeadlineChecker(unittest.TestCase):
    ...
    def test_deadline_checker_warnings(self):
        ...
        try:
            ...
            for char in output:
                self.assertLess(ord(char), 0x1F600, "Found an emoji or high unicode character in output!")
