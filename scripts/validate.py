...
Checks:
  ...
  5. regulatory-deadlines.json is well formed, mandatory fields are present, and dates are in YYYY-MM-DD format.
...
REQUIRED_PATTERN = ["id", "platform", "guideline", "title", "severity", "detection", "fix"]
SEVERITIES = {"critical", "high", "medium", "low"}
PLATFORMS = {"apple", "google", "both"}

REQUIRED_DEADLINE = [
    "id", "jurisdiction", "law", "requirement", "effective_date",
...
def validate_date(date_str, field_name, item_id):
    try:
        ...
        errors.append(f"deadline '{item_id}' has invalid {field_name} '{date_str}', must be YYYY-MM-DD format")
...
def main():
                ...
                for d in deadlines:
                    ...
                    for f in REQUIRED_DEADLINE:
                        if f not in d or d[f] is None:
                            errors.append(f"deadline '{did}' missing required field '{f}'")

                    if d.get("priority") not in SEVERITIES:
                        errors.append(f"deadline '{did}' has invalid priority '{d.get('priority')}'")

                    # Validate date formats
                    for date_field in ["effective_date", "mandatory_date", "enforcement_date"]:
    ...
    return finish(len(patterns), len(recipes) if 'recipes' in locals() else 0, len(deadlines) if 'deadlines' in locals() else 0)
...
def finish(npat=0, nrec=0, ndead=0):
    ...
    if errors:
        ...
        return 1
    print(f"validate. OK. {npat} patterns, {nrec} recipes, {ndead} deadlines, {len(warnings)} warning(s), 0 errors")
    return 0
